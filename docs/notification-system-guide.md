...
نظام إشعارات شامل ومتطور مصمم خصيصاً لنظام الأرشيف مع دعم كامل للغة العربية والواجهة RTL.

---

## ✨ التحديث الجديد - أيقونة الإشعارات في شريط التنقل

### 🎯 الميزة الجديدة المضافة
- **أيقونة ثابتة**: تظهر بجانب اسم المستخدم في أعلى كل صفحة
- **شارة العدد**: تعرض عدد الإشعارات غير المقروءة مع تأثير نبض جذاب
- **قائمة منسدلة تفاعلية**: عرض آخر 10 إشعارات مع إمكانية التفاعل المباشر
- **تحديث فوري**: مراقبة مستمرة للإشعارات الجديدة عبر Firebase Firestore
- **تصميم عربي متقن**: دعم كامل لـ RTL والخطوط العربية

### 🛠️ المكونات المضافة
```
public/assets/js/
├── notification-badge.js      # شارة الإشعارات والقائمة المنسدلة
├── notification-service.js    # خدمة إدارة الإشعارات المتطورة
...
### 1. نظام شارة الإشعارات المحدث
- **عرض في الوقت الفعلي**: تحديث فوري للإشعارات الجديدة
- **شارة ذكية**: تختفي عند عدم وجود إشعارات، تظهر العدد مع تأثير نبض
- **قائمة تفاعلية**: انقر على أي إشعار للانتقال إلى الصفحة ذات الصلة
- **إدارة سهلة**: تحديد الكل كمقروء، تنظيف تلقائي للإشعارات القديمة
- **إحصائيات لحظية**: عرض إجمالي الإشعارات والمقروءة وغير المقروءة

### 2. أنواع الإشعارات المدعومة
1. **تحميل ملف** (`file_upload`) 📤
   - إشعار المدراء عند تحميل ملف جديد
   - معلومات: اسم الملف، المستخدم، الوقت

2. **نقل ملف** (`file_movement`) 🔄
   - إشعار عند نقل ملف بين المواقع  
   - معلومات: الموقع السابق والجديد

3. **دعوة مستخدم** (`user_invitation`) 👥
   - إشعار المستخدم المدعو والمدراء
   - معلومات: البريد الإلكتروني والدور

4. **تنبيه النظام** (`system_alert`) ⚠️
   - تنبيهات الصيانة والتحديثات
   - أولوية عالية للأحداث المهمة

5. **تنبيه أمني** (`security_alert`) 🛡️
   - إنذارات أمنية فورية
   - محاولات دخول مشبوهة

6. **النسخ الاحتياطي** (`backup_complete`) 💾
   - تأكيد إتمام النسخ الاحتياطية
   - تفاصيل الحجم والمحتوى

### 3. نظام التنبيهات المتقدم (Advanced Alerts)
