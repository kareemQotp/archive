...
const {buildResponse, checkRateLimit, verifyAppCheck, isSystemOperatorRole, isSuperAdminRole} = require("../utils/helpers");
...
async function getCallerRole(request) {
  ...
  try {
    ...
    // Fall back to token claims when the user profile cannot be read.
  }
...
exports.getAdminPortalConfig = onCall({
  ...
  try {
    ...
    if (!isSystemOperatorRole(callerRole)) {
