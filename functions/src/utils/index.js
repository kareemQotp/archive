...
const {buildResponse, requireAuth, getUserRole, checkRateLimit, verifyAppCheck, isSystemOperatorRole, isSuperAdminRole} = require("./helpers");
...
exports.sendNotification = onCall({
  ...
  try {
    ...
    if (!isSystemOperatorRole(senderRole) && senderRole !== ROLES.ARCHIVE_OFFICER) throw new HttpsError("permission-denied", "Insufficient permissions");
    ...
    logger.error("Error sending notification:", error); if (error instanceof HttpsError) throw error; throw new HttpsError("internal", error.message);
  }
});
...
exports.generateSystemReport = onCall({
  ...
  try {
    ...
    if (!isSystemOperatorRole(userRole)) {
...
exports.listBackups = onCall({enforceAppCheck: false}, async (request) => {
  try {
    ...
    if (!isSuperAdminRole(role)) throw new HttpsError("permission-denied", "Super admin access required");
 ...
 */
exports.cleanupOldData = onSchedule("0 2 * * *", async (_event) => {
  try {
 ...
 */
exports.generateDailyStats = onSchedule("0 0 * * *", async (_event) => {
  try {
 ...
 */
async function rawHealthCheck(_request) {
  try {
    // Test database connection
    await db.collection(COLLECTIONS.SYSTEM_SETTINGS).doc("health_check").get();
...
exports.refreshUserClaims = onCall({enforceAppCheck: false}, async (request) => {
  try {
    ...
    if (!isSuperAdminRole(callerRole)) throw new HttpsError("permission-denied", "Super admin access required");
...
exports.receiveFileMovement = onCall({enforceAppCheck: false}, async (request) => {
  try {
    ...
    if (data.toDepartment) {
      ...
      if (!isSystemOperatorRole(role) && dept !== data.toDepartment) {
...
exports.restoreDeletedDocument = onCall({enforceAppCheck: false}, async (request) => {
  try {
    ...
    const data = doc.data();
    const role = await getUserRole(uid);
    if (!isSystemOperatorRole(role) && data.createdBy !== uid) {
      throw new HttpsError("permission-denied", "Not allowed");
    }
